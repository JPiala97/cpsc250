...
\section{Recursive Binary Search for a Name}

\begin{lstlisting}[language=Python]
def contains_name(names, target, low, high):
    ...
    if names[middle] == target:
        ...
        return contains_name(names, target, low, middle - 1)

    else:
        return contains_name(names, target, middle + 1, high)
\end{lstlisting}

\section{Example: Is Moe in the List?}

\begin{lstlisting}[language=Python]
names = ["Alice", "Charlie", "Dave", "Frieda", "Kallie",
         "Moe", "Paul", "Sarah", "Xavier"]

answer = contains_name(names, "Moe", 0, len(names) - 1)
...
\text{low} & \text{high} & \text{middle} & \text{name at middle} & \text{decision} \\
\hline
0 & 8 & 4 & \text{Kallie} & \text{Moe comes after Kallie} \\
5 & 8 & 6 & \text{Paul} & \text{Moe comes before Paul} \\
5 & 5 & 5 & \text{Moe} & \text{Found it!}
\end{array}
\]
...
\section{Example: Is Raoul in the List?}

\begin{lstlisting}[language=Python]
answer = contains_name(names, "Raoul", 0, len(names) - 1)
...
\text{low} & \text{high} & \text{middle} & \text{name at middle} & \text{decision} \\
\hline
0 & 8 & 4 & \text{Kallie} & \text{Raoul comes after Kallie} \\
5 & 8 & 6 & \text{Paul} & \text{Raoul comes after Paul} \\
7 & 8 & 7 & \text{Sarah} & \text{Raoul comes before Sarah} \\
7 & 6 & - & - & \text{No items left to search}
\end{array}
\]
